...
class ExamReportController extends Controller
{
    ...
    {
        $data['exams'] = Exam::Active()->CurrentSession()->has('ExamRemarks')->with('AcademicSession')->get();
        ...
        if ($request->ajax()) {
            $students = Student::select('students.id', 'students.gr_no', 'students.name')
                ...
                ->join('academic_session_history', function ($join) {
                    $join->on('students.id', '=', 'academic_session_history.student_id')->where('academic_session_history.academic_session_id', Auth::user()->academic_session);
                })
                ->get();
    ...
    public function GetExamTabulation(Request $request)
    {
        ...
        $data['transcripts'] = ExamRemark::where([
            ...
            ->with([
                'Student' => function ($qry) use ($data) {
                    $qry->select('id', 'name', 'gr_no', 'father_name');
                    $qry->with([
                        'StudentAttendance' => function ($qry) use ($data) {
    ...
    public function ResultTranscript(Request $request)
    {
        ...
        if ($data['selected_exams']->count() == 0) {
            return redirect('exam-reports')->with([
                'toastrmsg' => [
                    'type' => 'error',
                    'title' => __('modules.exam_report_results_title'),
        ...
        foreach ($data['selected_exams'] as $exam) {
            $data['attendance'][$exam->id] = StudentAttendance::select('id', 'student_id', 'status', 'date')
                ...
                ->whereBetween('date', [$exam->getRawOriginal('start_date'), $exam->getRawOriginal('end_date')])
                ->get();
        }
        ...
        if ($AcademicSessionHistory == null) {
            return redirect('exam-reports')->with([
                'toastrmsg' => [
                    'type' => 'error',
                    'title' => __('modules.exam_report_results_title'),
                    'msg' => __('modules.exams_student_not_found'),
                ],
            ]);
        }
        ...
        if ($data['results'][0] == null) {
            return redirect('exam-reports')->with([
                'toastrmsg' => [
                    'type' => 'warning',
                    'title' => __('modules.exam_report_results_title'),
                    'msg' => __('modules.exams_data_not_found'),
                ],
            ]);
        }
        $data['grades'] = Grade::all();
    ...
    public function UpdateRank(Request $request)
    {
        if ($request->ajax()) {
            ...
            if ($validator->fails()) {
                return [
                    'type' => 'error',
                    'title' => __('modules.exam_report_student_results_title'),
